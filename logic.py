...
def init(screen):
    ...
    height -= ground
# класс, описывающий вектор(для траектории полета птиц и прочей физики)
class Vector:
    def __init__(self, magnitude=0, angle=radians(0)):
        self.magnitude = magnitude
        self.angle = angle

# расчет нового вектора при столкновении
...
# класс, описывающий свиней
class Pig:
    def __init__(self, x, y, r, v=None, loaded = False, color=(0, 0, 0)):
        self.x = x
        self.y = y
        self.r = r
        if v == None:
    ...
    def draw(self):
        ...
        if (not self.animate_count%20) and (not self.isDead):
            self.image = random.choice([self.pig1_image, self.pig2_image])
...
class Bird:
    def __init__(self, num, x, y, r, v=None, loaded = False, color=(0, 0, 140)):
        self.x = x
        self.y = y
        self.r = r
        if v == None:
            ...
            self.velocity = v
        if num == 1:
            self.bird_image = pygame.image.load("Images/red_bird.png")
        elif num == 2:
            self.bird_image = pygame.image.load("Images/blue_bird.png")
        elif num == 3:
            self.bird_image = pygame.image.load("Images/yellow_bird.png")
        else:
            self.bird_image = pygame.image.load("Images/red_bird.png")
    ...
    def draw(self):
        ...
        display.blit(self.bird_image, (self.x - self.r, self.y - self.r))

    # смерть(после того, как остановилась после запуска)
    def dead(self):
        self.isDead = True
        self.image = self.pig_dead

    # перемещение при запуске и столкновениях
    def move(self):
        self.velocity = add_vectors(self.velocity, gravity)
    ...
    def project_path(self):
        if self.loaded:
            path = []
            ball = Bird(3, self.x, self.y, self.r, self.velocity)
            for i in range(30):
                ball.move()
                if i%5 == 0:
...
# класс рогатки
class Slingshot:
    def __init__(self, x, y, w, h, color=( 130, 70, 11 )):
        self.x = x
        self.y = y
        self.w = w
        self.h = h
